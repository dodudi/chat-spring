...
public class DefaultProfileEventPublisher implements ProfileEventPublisher {
    ...
    @Override
    public void publish(List<UUID> roomIds, ProfileUpdatedEvent event) {
        String payload;
        try {
            payload = objectMapper.writeValueAsString(event);
        } catch (JsonProcessingException e) {
            log.error("[REDIS_PUB_FAIL] profileId={}", event.profileId(), e);
            return;
        }
        for (UUID roomId : roomIds) {
            try {
                ...
                log.error("[REDIS_PUB_FAIL] roomId={} profileId={}", roomId, event.profileId(), e);
            }
        }
    }
}
