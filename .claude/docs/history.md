...
| M-04 | 방 목록에 마지막 메시지·시간 표시 | ✅ `findLastMessages` + `countUnreadByRoomIds` 배치 쿼리 → `getMyRooms` |
...
| S-03 | 개인 알림 채널 수신 (초대 알림) | ✅ `pubsub:user:{userId}` → `/user/{userId}/queue/notifications` |
| S-04 | STOMP 연결 시 대기 중 초대 일괄 전송 | ✅ `SessionEventHandler.handleConnect` → `InvitationReader` → `/user/{userId}/queue/notifications` |
...
| ID | 요구사항 | 상태 |
|----|---------|------|
| I-01 | 방장이 GROUP·PUBLIC 방 멤버 초대 (DM 불가) | ✅ `POST /api/v1/rooms/{roomId}/invitations` |
| I-02 | 초대 수락 — 프로필 선택, 인원 검증, 그룹 등록 | ✅ `PATCH /api/v1/invitations/{id}/accept` |
| I-03 | 초대 거절 | ✅ `PATCH /api/v1/invitations/{id}/reject` |
| I-04 | 대기 중 초대 목록 조회 | ✅ `GET /api/v1/invitations` |
| I-05 | 초대 발송 시 실시간 알림 (Redis pub/sub) | ✅ `afterCommit` → `pubsub:user:{inviteeId}` |
| I-06 | STOMP 연결 시 대기 초대 일괄 전송 | ✅ `SessionEventHandler.handleConnect` |
...
| ID | 요구사항 | 상태 |
|----|---------|------|
| L-01 | GROUP·PUBLIC 방 방장이 초대 링크 생성 (DM 불가, 만료 시간 선택) | ✅ `POST /api/v1/rooms/{roomId}/invite-links` |
| L-02 | 방장이 초대 링크 비활성화 | ✅ `DELETE /api/v1/invite-links/{id}` |
| L-03 | 초대 링크로 방 입장 (비밀번호 우회, 강퇴 이력 해제) | ✅ `POST /api/v1/invite-links/{token}/join` |
| L-04 | 만료된 링크 차단 | ✅ L001 INVITE_LINK_EXPIRED |
| L-05 | 비활성화된 링크 차단 | ✅ L002 INVITE_LINK_INACTIVE |
...
| MSG-03 | 본인 메시지 수정 (`PATCH /messages/{id}`, is_edited 플래그) | ✅ afterCommit → `MESSAGE_EDITED` 이벤트 브로드캐스트 |
| MSG-04 | 본인 메시지 철회 soft delete (`DELETE /messages/{id}`) | ✅ afterCommit → `MESSAGE_DELETED` 이벤트 브로드캐스트 |
...
| ID | 요구사항 | 상태 |
|----|---------|------|
| P-01 | 입장 후 사용 프로필 변경 | ✅ `PATCH /api/v1/rooms/{roomId}/members/me/profile` |
...
| ID | 요구사항 | 상태 |
|----|---------|------|
| PF-01 | 닉네임 수정 시 해당 프로필을 사용 중인 모든 채팅방에 즉시 반영 | ✅ `PATCH /api/v1/profiles/{id}` afterCommit → `pubsub:room:{roomId}` → `/topic/rooms/{roomId}` (`PROFILE_UPDATED` 이벤트) |
...
| ID | 요구사항 | 상태 |
|----|---------|------|
| BA-01 | 메시지 30일 보관 — 오래된 메시지 자동 삭제 | ✅ `MessageRetentionJob` 매일 03:00 |
| BA-02 | 빈 채팅방 자동 삭제 | ✅ `EmptyRoomCleanupJob` 매일 03:30 |
